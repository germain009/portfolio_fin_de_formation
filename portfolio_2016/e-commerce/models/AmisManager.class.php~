...
class AmisManager
{
        ...
        public function getAmisExp($pseudo_dest)
	{
        ...
        //recupere la liste de tout les amis d'un membre
        public function getAmis($pseudo_dest)
	{
            $list = [];
	    $query = "SELECT * FROM amis WHERE pseudo_exp ='".$pseudo_dest."'
                      AND active = 1 
        ...
        public function getAmis_pagination($pseudo,$c_page,$parPage)
	{
            $list = [];
            
	    $query = "SELECT * FROM amis WHERE pseudo_exp ='".$pseudo."'
                      AND active = 1 
                      OR
                      pseudo_dest = '".$pseudo."'
                      AND active = 1 
                      LIMIT ".(($c_page - 1) * $parPage).", $parPage  ";
	    $res = mysqli_query($this->link, $query);
	    while ($invit = mysqli_fetch_object($res, "Amis", [$this->link]))
            {
                $list[] = $invit;
        ...
        public function accepterInvitation($info1,$info2)
	{
         ...
         public function invitationValidée($info)
	 {
        ...
        //pour refuser une invitation c'est l'envoyeur qui la supprimera
        public function refuseInvitation($info1,$info2)
	{
            $pseudo_exp = $info1;
	    $pseudo_dest = $info2;
            $date = new DateTime();
	    ...
	    if (!$res)
         ...
         //recupere les invitation refusées
         public function getInvitationRefuse($info)
          {
              $list = [];
	      $query = "SELECT pseudo_dest FROM amis WHERE pseudo_exp ='".$info."'
                        AND date_confirmation > date_vue 
